...
ErrorStatus RTC_DeInit(void)
{
  ...
  {
    /* Reset TR registers */
    RTC_TR1 = RTC_TR1_RESET_VALUE;
    RTC_TR2 = RTC_TR2_RESET_VALUE;
    RTC_TR3 = RTC_TR3_RESET_VALUE;

    /* Reset DR registers */
    RTC_DR1 = RTC_DR1_RESET_VALUE;
    RTC_DR2 = RTC_DR2_RESET_VALUE;
    RTC_DR3 = RTC_DR3_RESET_VALUE;

    /* Reset SPER & ARPER registers */
    RTC_SPRERH = RTC_SPRERH_RESET_VALUE;
    RTC_SPRERL = RTC_SPRERL_RESET_VALUE;
    RTC_APRER  = RTC_APRER_RESET_VALUE;

    RTC_TCR1 = RTC_TCR1_RESET_VALUE;
    RTC_TCR2 = RTC_TCR2_RESET_VALUE;

    /* Reset All CR1 bits except CR1[2:0] */

    RTC_CR1 = RTC_CR1_RESET_VALUE;
    RTC_CR2 = RTC_CR2_RESET_VALUE;
    RTC_CR3 = RTC_CR3_RESET_VALUE;

    /* Wait till RTC WUTWF flag is set or if Time out is reached exit */
    while (((RTC_ISR1 & RTC_ISR1_WUTWF) == RESET) && ( wutwfcount != WUTWF_TIMEOUT))
    {
      wutwfcount++;
      RTC_ISR1 = 0;
    }

    if ((RTC_ISR1 & RTC_ISR1_WUTWF) == RESET)
    {
      status = ERROR;
      /* Enable the write protection for RTC registers */
      RTC_WPR = 0xFF; 
    ...
    else
    {
      ...
      RTC_WUTRH = RTC_WUTRH_RESET_VALUE;
      RTC_WUTRL = RTC_WUTRL_RESET_VALUE;

      /* Reset ALARM registers */
      RTC_ALRMAR1 = RTC_ALRMAR1_RESET_VALUE;
      RTC_ALRMAR2 = RTC_ALRMAR2_RESET_VALUE;
      RTC_ALRMAR3 = RTC_ALRMAR3_RESET_VALUE;
      RTC_ALRMAR4 = RTC_ALRMAR4_RESET_VALUE;

      RTC_ALRMASSRH = RTC_ALRMASSRH_RESET_VALUE;
      RTC_ALRMASSRL = RTC_ALRMASSRL_RESET_VALUE;
      RTC_ALRMASSMSKR = RTC_ALRMASSMSKR_RESET_VALUE;

      /* Reset ISR register and exit initialization mode */
      RTC_ISR1 = (uint8_t)0x00;
      RTC_ISR2 = RTC_ISR2_RESET_VALUE;

      if ((RTC_ISR1 & RTC_ISR1_RECALPF) != RESET)
      {
        while (((RTC_ISR1 & RTC_ISR1_RECALPF) != RESET) && (recalpfcount != RECALPF_TIMEOUT))
        {
          recalpfcount++;
        }
      }
      if ((RTC_ISR1 & RTC_ISR1_RECALPF) == RESET)
      {
        RTC_CALRH = RTC_CALRH_RESET_VALUE;
        RTC_CALRL = RTC_CALRL_RESET_VALUE;
...
//RTC->WPR = 0x53;
  ...
  //RTC->CR1 &= ((uint8_t)~( RTC_CR1_FMT ));
    RTC_CR1 &= ((uint8_t)~( RTC_CR1_FMT ));
    /* Set RTC_CR1 register */
  //RTC->CR1 |=  ((uint8_t)(RTC_InitStruct->RTC_HourFormat));
    RTC_CR1 |=  ((uint8_t)(RTC_InitStruct->RTC_HourFormat));
  ...
  //RTC->APRER =  (uint8_t)(RTC_InitStruct->RTC_AsynchPrediv);
    RTC_SPRERH = (uint8_t)(RTC_InitStruct->RTC_SynchPrediv >> 8);
    RTC_SPRERL = (uint8_t)(RTC_InitStruct->RTC_SynchPrediv);
    RTC_APRER =  (uint8_t)(RTC_InitStruct->RTC_AsynchPrediv);
    /* Exit Initialization mode */
  //RTC->ISR1 &= (uint8_t)~RTC_ISR1_INIT;
    RTC_ISR1 &= (uint8_t)~RTC_ISR1_INIT;
    status = SUCCESS;
  }
...
void RTC_WriteProtectionCmd(FunctionalState NewState)
{
  ...
  {
    /* Enable the write protection for RTC registers */
    RTC_WPR = RTC_WPR_EnableKey;
  }
  else
  {
    /* Disable the write protection for RTC registers */
    RTC_WPR = RTC_WPR_DisableKey1;
    RTC_WPR = RTC_WPR_DisableKey2;
  }
}
...
ErrorStatus RTC_EnterInitMode(void)
{
  ...
  if ((RTC_ISR1 & RTC_ISR1_INITF) == RESET)
  {
    /* Set the Initialization mode */
    RTC_ISR1 = (uint8_t)RTC_ISR1_INIT;

    /* Wait until INITF flag is set */
    while (((RTC_ISR1 & RTC_ISR1_INITF) == RESET) && ( initfcount != INITF_TIMEOUT))
    {
      initfcount++;
    }
  }
...
ErrorStatus RTC_WaitForSynchro(void)
{
  ...
  RTC_ISR1 &= (uint8_t)~(RTC_ISR1_RSF | RTC_ISR1_INIT);

  /* Wait the registers to be synchronised */
  while (((RTC_ISR1 & RTC_ISR1_RSF) == RESET) && ( rsfcount != RSF_TIMEOUT))
  {
    rsfcount++;
  }

  /* Check if RSF flag occurs*/
  if ((RTC_ISR1 & RTC_ISR1_RSF) != RESET)
  {
    status = SUCCESS;
  }
  else
...
void RTC_BypassShadowCmd(FunctionalState NewState)
{
  ...
  {
    /* Set the BYPSHAD bit */
    RTC_CR1 |= (uint8_t)RTC_CR1_BYPSHAD;
  }
  else
  {
...
*/
	RTC_TR1 = (uint8_t)(RTC_TimeStruct->RTC_Seconds);
      RTC_TR2 = (uint8_t)(RTC_TimeStruct->RTC_Minutes) ;
      RTC_TR3 = (uint8_t)( temp | RTC_TimeStruct->RTC_Hours) ;
    }
    else
    {
/*
...
*/
      RTC_TR1 = (uint8_t)(ByteToBcd2(RTC_TimeStruct->RTC_Seconds));
      RTC_TR2 = (uint8_t)(ByteToBcd2(RTC_TimeStruct->RTC_Minutes)) ;
      RTC_TR3 = (uint8_t)( temp | ByteToBcd2(RTC_TimeStruct->RTC_Hours));
    }
    ...
    if ((RTC_CR1 & RTC_CR1_BYPSHAD) == RESET)
    {
      if (RTC_WaitForSynchro() == ERROR)
      {
        status = ERROR;
                 ...
                 RTC_TimeTypeDef* RTC_TimeStruct)
{
  ...
  /* Get RTC seconds */
  RTC_TimeStruct->RTC_Seconds = RTC_TR1;

  /* Get RTC Minutes */
  RTC_TimeStruct->RTC_Minutes = RTC_TR2;

  /* Get the RTC_TR3 register */
  tmpreg = (uint8_t)RTC_TR3;
                        ...
                        RTC_DateTypeDef* RTC_DateStruct)
{
  ...
  else
  {
    ...
    if (RTC_Format != RTC_Format_BIN)
    {
      RTC_DR1 = (uint8_t)(RTC_DateStruct->RTC_Date);
      RTC_DR2 = (uint8_t)((RTC_DateStruct->RTC_Month) | (uint8_t)((RTC_DateStruct->RTC_WeekDay) << 5));
      RTC_DR3 = (uint8_t)((RTC_DateStruct->RTC_Year));
    }
    else
    {
      RTC_DR1 = (uint8_t)(ByteToBcd2 ((uint8_t)RTC_DateStruct->RTC_Date));
      RTC_DR2 = (uint8_t)((ByteToBcd2((uint8_t)RTC_DateStruct->RTC_Month)) | (uint8_t)((RTC_DateStruct->RTC_WeekDay) << 5));
      RTC_DR3 = (uint8_t)(ByteToBcd2((uint8_t)RTC_DateStruct->RTC_Year));
    }
                 ...
                 RTC_DateTypeDef* RTC_DateStruct)
{
  ...
  /* Fill the structure fields with the read parameters */
  (void) (RTC_TR1) ;
  RTC_DateStruct->RTC_Date = (uint8_t)(RTC_DR1);
  tmpreg = (uint8_t)RTC_DR2;
  RTC_DateStruct->RTC_Year = (uint8_t)(RTC_DR3);
                  ...
                  RTC_AlarmTypeDef* RTC_AlarmStruct)
{
  ...
  tmpreg1 = (uint8_t)RTC_ALRMAR1;
  tmpreg2 = (uint8_t)RTC_ALRMAR2;
  tmpreg3 = (uint8_t)RTC_ALRMAR3;
  tmpreg4 = (uint8_t)RTC_ALRMAR4;
...
ErrorStatus RTC_AlarmCmd(FunctionalState NewState)
{
  ...
  {  /* Disable the Alarm */
    ...
    temp1 = (uint8_t)(RTC_ISR1 & RTC_ISR1_ALRAWF);
    while ((alrawfcount != ALRAWF_TIMEOUT) && (temp1 == RESET))
    {
      alrawfcount++;
    }

    if ((RTC_ISR1 &  RTC_ISR1_ALRAWF) == RESET)
    {
      status = ERROR;
    }
    else
                                     ...
                                     RTC_AlarmSubSecondMask_TypeDef RTC_AlarmSubSecondMask)
{
  ...
  {
    /* save  Alarm status */
    alarmstatus = (uint8_t)(RTC_CR2 | RTC_CR2_ALRAE);

    /* Disable the Alarm */
    RTC_CR2 &= (uint8_t)~(RTC_CR2_ALRAE);

    /* Configure the Alarm register */
    RTC_ALRMASSRH = (uint8_t)(RTC_AlarmSubSecondValue >> 8);
    RTC_ALRMASSRL = (uint8_t)(RTC_AlarmSubSecondValue);
    RTC_ALRMASSMSKR = (uint8_t)RTC_AlarmSubSecondMask;
                              ...
                              RTC_StoreOperation_TypeDef RTC_StoreOperation)
{
  ...
  RTC_CR3 &= (uint8_t)~(RTC_CR3_BCK);

  /* Configure the RTC_CR3 register */
  RTC_CR3 |= (uint8_t)((uint8_t)RTC_DayLightSaving | (uint8_t)RTC_StoreOperation);
...
RTC_StoreOperation_TypeDef  RTC_GetStoreOperation(void)
{
  ...
  return (RTC_StoreOperation_TypeDef)(RTC_CR3 & RTC_CR3_BCK);
}
                      ...
                      RTC_OutputPolarity_TypeDef RTC_OutputPolarity)
{
  ...
  /* Clear the bits to be configured */
  RTC_CR3 &= (uint8_t)~(RTC_CR3_OSEL | RTC_CR3_POL);

  /* Configure the output selection and polarity */
  RTC_CR3 |= (uint8_t)((uint8_t)RTC_Output | (uint8_t)RTC_OutputPolarity);
                                   ...
                                   uint16_t RTC_ShiftSubFS)
{
  ...
  if ((RTC_ISR1 & RTC_ISR1_SHPF) != RESET)
  {
    /* wait until the shift is completed*/
    while (((RTC_ISR1 & RTC_ISR1_SHPF) != RESET) && (shpfcount != SHPF_TIMEOUT))
    {
      shpfcount++;
    }
  }
  ...
  if ((RTC_ISR1 & RTC_ISR1_SHPF) == RESET)
  {
    ...
    RTC_SHIFTRH = (uint8_t)(shiftrhreg);
    RTC_SHIFTRL = (uint8_t)(RTC_ShiftSubFS);
                                  ...
                                  uint16_t RTC_SmouthCalibMinusPulsesValue)
{
  ...
  if ((RTC_ISR1 & RTC_ISR1_RECALPF) == RESET)
  {
    ...
    RTC_CALRH = (uint8_t)((uint8_t)((uint8_t)RTC_SmoothCalibPeriod | \
                           (uint8_t)RTC_SmoothCalibPlusPulses) | \
                           (uint8_t)((uint16_t)RTC_SmouthCalibMinusPulsesValue >> 8));
    RTC_CALRL = (uint8_t)(RTC_SmouthCalibMinusPulsesValue);
                           ...
                           RTC_TamperLevel_TypeDef RTC_TamperLevel)
{
  ...
  if (RTC_TamperLevel != RTC_TamperLevel_Low)
  {
    ...
    RTC_TCR1 |= (uint8_t)(RTC_Tamper << 1);
  }
  else
  {
    /* Disable the selected Tampers */
    RTC_TCR1 &= (uint8_t)~(uint8_t)(RTC_Tamper << 1);
  }
...
void RTC_TamperFilterConfig(RTC_TamperFilter_TypeDef RTC_TamperFilter)
{
  ...
  RTC_TCR2 &= (uint8_t)~(RTC_TCR2_TAMPFLT);

  /* Configure the RTC_TCR register */
  RTC_TCR2 |= (uint8_t)RTC_TamperFilter;
...
void RTC_TamperSamplingFreqConfig(RTC_TamperSamplingFreq_TypeDef RTC_TamperSamplingFreq)
{
  ...
  /* Clear flags before configuration */
  RTC_TCR2 &= (uint8_t)~(RTC_TCR2_TAMPFREQ);

  /* Configure the RTC_TCR register */
  RTC_TCR2 |= (uint8_t)RTC_TamperSamplingFreq;
...
void RTC_TamperPinsPrechargeDuration(RTC_TamperPrechargeDuration_TypeDef RTC_TamperPrechargeDuration)
{
  ...
  RTC_TCR2 &= (uint8_t)~(RTC_TCR2_TAMPPUDIS | RTC_TCR2_TAMPPRCH);

  /* Configure the Tampers Precharge Duration  and pull-up New settings*/
  RTC_TCR2 |= (uint8_t)RTC_TamperPrechargeDuration;
...
void RTC_ITConfig(RTC_IT_TypeDef RTC_IT, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    RTC_CR2 |= (uint8_t)((uint16_t)RTC_IT & (uint16_t)0x00F0);
    RTC_TCR1 |= (uint8_t)((uint16_t)RTC_IT & RTC_TCR1_TAMPIE);
  }
  else
  {
    /* Disable the Interrupts */
    RTC_CR2  &= (uint8_t)~(uint8_t)((uint16_t)RTC_IT & (uint16_t)0x00F0);
    RTC_TCR1 &= (uint8_t)~(uint8_t)((uint16_t)RTC_IT & (uint16_t)RTC_TCR1_TAMPIE);
  }
...
FlagStatus RTC_GetFlagStatus(RTC_Flag_TypeDef RTC_FLAG)
{
  ...
  /* Get all the flags */
  tmpreg2 = (uint16_t)((uint16_t)RTC_ISR1 << 8);
  tmpreg1 = (uint16_t)((uint16_t)((uint16_t)(RTC_ISR2)) | tmpreg2);
...
void RTC_ClearFlag(RTC_Flag_TypeDef RTC_FLAG)
{
  ...
  RTC_ISR2 = (uint8_t)~((uint8_t)RTC_FLAG) ;
  RTC_ISR1 = (uint8_t)(((uint8_t)~(uint8_t)((uint16_t)RTC_FLAG >> (uint8_t)8)) & ((uint8_t)~(uint8_t)(RTC_ISR1_INIT)));
}
...
ITStatus RTC_GetITStatus(RTC_IT_TypeDef RTC_IT)
{
  ...
  enablestatus = (uint8_t)(RTC_CR2 & (uint16_t)RTC_IT);

  /* Get the Interrupt pending bit */
  tmpreg = (uint8_t)(RTC_ISR2 & (uint8_t)((uint16_t)RTC_IT >> 4));
...
void RTC_ClearITPendingBit(RTC_IT_TypeDef RTC_IT)
{
  ...
  RTC_ISR2 = (uint8_t)~(uint8_t)((uint16_t)RTC_IT >> 4);
}
